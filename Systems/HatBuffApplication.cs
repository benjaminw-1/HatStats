...
{
    private static Dictionary<Entity, PrefabGUID> _lastEquippedHelmet = new();
    ...
    public static void Prefix(ReactToInventoryChangedSystem __instance)
    {
        ...
        try
        {
            foreach (var entity in entities)
            {
                ...
                if (previousHelmetGuid.GuidHash != 0)
                {
                    ...
                    //buffEntity.TryGetBuffer<ModifyUnitStatBuff_DOTS>(out var modifyStatBuffer);

                    buffEntity.TryDestroyBuff();


                    FixedString512Bytes msg = $"Removed hat (was GUID: {previousHelmetGuid.GuidHash})";
